...
Neural-Abyss is a Python/PyTorch grid simulation with per-agent neural policies, a vectorized tick engine, an interactive `pygame-ce` viewer, and built-in persistence for long runs. The repository combines world simulation, policy inference, per-agent PPO training, checkpoint/resume, telemetry, and optional video recording in one codebase. Runtime configuration is environment-driven through `FWS_*` variables, and fresh runs are written into timestamped directories under `results/` by default or a custom `FWS_RESULTS_DIR` when provided.
...
| UI | `pygame-ce` viewer in `ui/viewer.py` |
| Entry point | `main.py` |
| Default grid | `100x100` |
| Default initial population | `150` agents per team |
| Observation width | `283` |
| Action count | `41` |
| Output root | `results/` by default, overridden by `FWS_RESULTS_DIR` |
...
- **Per-agent actor-critic brains.** `agent/mlp_brain.py` defines five MLP variants behind one shared observation contract and actor/critic interface.
...
- Read `main.py` first for runtime orchestration and end-to-end control flow.
- Read `engine/tick.py` next for simulation semantics.
...
The source tree does not include a lockfile. The imports show these direct dependencies:

- `torch`
- `numpy`
- `pygame-ce`
- `opencv-python` for video recording via `cv2`
...
python -m pip install torch numpy pygame-ce opencv-python pytest
```
...
The default configuration enables the UI. The viewer is implemented in `ui/viewer.py` and advances the simulation through `engine.run_tick()`.
...
- `FWS_RESULTS_DIR` - base directory for fresh-run outputs
- `FWS_UI` - enable or disable the viewer
...
A normal run creates `<FWS_RESULTS_DIR>/sim_YYYY-MM-DD_HH-MM-SS/`, which defaults to `results/sim_YYYY-MM-DD_HH-MM-SS/`. The main output layout is:
